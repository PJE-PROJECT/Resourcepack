...
void main() {
    ...
    if (texelFetch(Sampler0, ivec2(0, 0), 0).a == 254.0 / 255.0) {
        ...
        if (RatioRatio < 1.0) {
            texCoord0 = (gl_Position.xy * vec2(1.0, RatioRatio) * vec2(0.5, -0.5) + 0.5);
        } else {
            texCoord0 = (gl_Position.xy * vec2(1.0 / RatioRatio, 1.0) * vec2(0.5, -0.5) + 0.5);
        }
    } else {
        texCoord0 = UV0;
    }
