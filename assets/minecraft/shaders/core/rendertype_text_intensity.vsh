...
void main() {
    ...
    if (iColor == ivec3(78, 92, 36) && (
        ...
        Position.z == 2000.0     // Scoreboard Sidebar (Has no shadow, remove tint for consistency)
    )) { // Regular text
        vertexColor.rgb = texelFetch(Sampler2, UV2 / 16, 0).rgb; // Remove color from no shadow marker
    } else if (iColor == ivec3(19, 23, 9) && (
        Position.z == 2200.0 || // Actionbar
        Position.z == 2400.0 || // Subtitle | Title
        Position.z == 50.0 ||   // Opened Chat
        Position.z == 2650.0 || // Closed Chat
        Position.z == 200.0 ||  // Advancement Screen
        Position.z == 400.0 ||  // Items
        Position.z == 1000.0 || // Bossbar
        Position.z == 2800.0    // Scoreboard List
    )) { // Shadow
        gl_Position = vec4(2.0, 2.0, 2.0, 1.0); // Move shadow off screen
    }
}
